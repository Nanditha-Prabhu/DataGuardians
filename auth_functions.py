...
def create_user_with_email_and_password(email, password, uid):
    ...
    headers = {"content-type": "application/json; charset=UTF-8" }
    data = json.dumps({"email": email, "password": password, "uid":uid, "returnSecureToken": True})
    request_object = requests.post(request_ref, headers=headers, data=data)
    raise_detailed_error(request_object)
    return request_object.json()
...
def create_account(email:str, password:str, uid:str, role:str) -> None:
    try:
        ...
        id_token = create_user_with_email_and_password(email,password,uid)['idToken']
